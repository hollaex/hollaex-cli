...
function generate_backend_passwords() {
  ...
  if [[ ! "$HOLLAEX_SECRET_REDIS_PASSWORD" ]] || [[ ! "$HOLLAEX_SECRET_PUBSUB_PASSWORD" ]]; then
  ...
  if [[ ! "$HOLLAEX_SECRET_DB_PASSWORD" ]]; then
  ...
  if [[ ! "$HOLLAEX_SECRET_MINIO_ROOT_PASSWORD" ]]; then

    export HOLLAEX_SECRET_MINIO_ROOT_PASSWORD=$(generate_random_values)

    sed -i.bak "s/HOLLAEX_SECRET_MINIO_ROOT_PASSWORD=.*/HOLLAEX_SECRET_MINIO_ROOT_PASSWORD=$HOLLAEX_SECRET_MINIO_ROOT_PASSWORD/" $SECRET_FILE_PATH
