  ...
  for DOCKER_CREDENTIAL_SERVER in "https://index.docker.io/v1/" "index.docker.io" "registry-1.docker.io" "https://registry-1.docker.io"; do
    ...
    if [[ "$DOCKER_CREDENTIAL_HELPER" ]] && command -v "docker-credential-$DOCKER_CREDENTIAL_HELPER" > /dev/null 2>&1; then
      ...
      if [[ "$DOCKER_REGISTRY_AUTH" ]]; then
        ...
        if ! DOCKER_REGISTRY_AUTH_DECODED=$(echo "$DOCKER_REGISTRY_AUTH" | base64 -d 2>/dev/null); then

          DOCKER_REGISTRY_AUTH_DECODED=$(echo "$DOCKER_REGISTRY_AUTH" | base64 -D 2>/dev/null)
...
function check_latest_hollaex_server_docker_tag() {

  local DOCKER_HUB_TOKEN_RESPONSE
  local DOCKER_HUB_TOKEN_HTTP_CODE
  local DOCKER_HUB_BEARER_TOKEN
  ...
  if ! DOCKER_HUB_TOKEN_HTTP_CODE=$(curl -s -S -o "$DOCKER_HUB_TOKEN_RESPONSE" -w "%{http_code}" \
      --user "$DOCKER_HUB_USERNAME:$DOCKER_HUB_PASSWORD" \
      "https://auth.docker.io/token?service=registry.docker.io&scope=repository:bitholla/hollaex-server:pull"); then
  ...
  # Docker pull reads the saved login from Docker's credential store directly.
  # These variables are only needed for the tag lookup request.
  unset DOCKER_HUB_USERNAME
  unset DOCKER_HUB_PASSWORD
  ...
  DOCKER_HUB_BEARER_TOKEN=$(jq -r '.token // .access_token // empty' "$DOCKER_HUB_TOKEN_RESPONSE")

  if [[ ! "$DOCKER_HUB_BEARER_TOKEN" ]]; then
  ...
  if ! DOCKER_HUB_TAG_HTTP_CODE=$(curl -s -S -o "$DOCKER_HUB_TAG_RESPONSE" -w "%{http_code}" \
      -H "Authorization: Bearer $DOCKER_HUB_BEARER_TOKEN" \
      "https://registry-1.docker.io/v2/bitholla/hollaex-server/tags/list?n=10000"); then
  ...
  HOLLAEX_SERVER_LATEST_DOCKER_TAG=$(jq -r \
    '[.tags[]? | select(test("^[0-9]+\\.[0-9]+\\.[0-9]+$")) | {tag: ., version: (split(".") | map(tonumber))}] | sort_by(.version) | last | .tag // empty' \
  ...
  if [[ ! "$HOLLAEX_SERVER_LATEST_DOCKER_TAG" ]]; then
    ...
    printf "\n\033[91mError: Failed to find a valid HollaEx Server Docker tag.\033[39m\n" >&2
    echo "Expected a version-only tag such as '2.19.10'." >&2
    return 1
...
function build_user_hollaex_core() {
  ...
  if [[ "$CUSTOM_GIT_REMOTE_URL" ]] || [[ "$UNSUPPORTED_GIT_BRANCH" ]] || [[ "$LOCAL_BUILD" ]]; then
    ...
    if ! ENVIRONMENT_USER_HOLLAEX_CORE_IMAGE_VERSION_OVERRIDE=$(check_latest_hollaex_server_docker_tag); then
    ...
    echo "Latest HollaEx Server Docker image: $ENVIRONMENT_USER_HOLLAEX_CORE_IMAGE_REGISTRY_OVERRIDE:$ENVIRONMENT_USER_HOLLAEX_CORE_IMAGE_VERSION_OVERRIDE"
