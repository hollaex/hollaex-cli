...
EOF
  ...
  local PARSE_CHARACTERS_FOR_WEB_IMAGE_TAG=$(echo $EXCHANGE_NAME_OVERRIDE | tr -dc '[:alnum:]' | tr -d ' ')
...
EOF
  ...
  for i in ${CONFIG_FILE_PATH[@]}; do
    ...
    sed -i.bak "s/ENVIRONMENT_KUBERNETES_WEB_IMAGE_VERSION=$ENVIRONMENT_KUBERNETES_WEB_IMAGE_VERSION/ENVIRONMENT_KUBERNETES_WEB_IMAGE_VERSION=$PARSE_CHARACTERS_FOR_WEB_IMAGE_TAG/" $CONFIGMAP_FILE_PATH
  ...
  export ENVIRONMENT_KUBERNETES_WEB_IMAGE_VERSION=$PARSE_CHARACTERS_FOR_WEB_IMAGE_TAG
