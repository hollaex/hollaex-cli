...
services:
  ...
  ${ENVIRONMENT_EXCHANGE_NAME}-db:
    image: ${ENVIRONMENT_DOCKER_IMAGE_POSTGRESQL_REGISTRY:-postgres}:${ENVIRONMENT_DOCKER_IMAGE_POSTGRESQL_VERSION:-13.0}
    restart: unless-stopped
    ports:
      - 5432:5432
    environment:
  ...
  ${ENVIRONMENT_EXCHANGE_NAME}-influxdb:
    image: ${ENVIRONMENT_DOCKER_IMAGE_INFLUXDB_REGISTRY:-influxdb}:${ENVIRONMENT_DOCKER_IMAGE_INFLUXDB_VERSION:-1.8.3-alpine}
    restart: unless-stopped
    ports:
      - 8086:8086
    environment:
...
function check_core_version_compatibility_range() {
  ...
  CURRENT_HOLLAEX_CORE_VERSION_CONVERTED=$(echo ${CURRENT_HOLLAEX_CORE_VERSION//.})
  HOLLAEX_CORE_MINIMUM_COMPATIBLE_CONVERTED=$(echo ${HOLLAEX_CORE_MINIMUM_COMPATIBLE//.})
  HOLLAEX_CORE_MAXIMUM_COMPATIBLE_CONVERTED=$(echo ${HOLLAEX_CORE_MAXIMUM_COMPATIBLE//.})

  if (( "$CURRENT_HOLLAEX_CORE_VERSION_CONVERTED" < "$HOLLAEX_CORE_MINIMUM_COMPATIBLE_CONVERTED" )) || (( "$CURRENT_HOLLAEX_CORE_VERSION_CONVERTED" > "$HOLLAEX_CORE_MAXIMUM_COMPATIBLE_CONVERTED" )); then
